...
pacman::p_load(data.table, lubridate, tidyr, dplyr, INLA)
...
excess.samps     <- fread("excess.csv") %>% mk.mat()
...
print("Distributions of monthly mortality predictions")
  ...
  print(".................................By Global")
  global.exc <- global.exp <- global.ac  <- array(dim = c(mx.m, 3)) # empty matrix to populate with distribution
  ...
  # 3) by region
  print(".................................By Region")
  region.exc <- region.exp <- region.ac <- array(dim = c(mx.m*6, 3))
  ...
  income.exc <- income.exp <- income.ac  <- array(dim = c(mx.m*4, 3))
...
who.mod.ests <- get.intervals("Model All")
